...
const obj = [
  {
    ...
    src: k2,
    msg: "I always get my raw materials from local sources to help support my local community!"
  },{
    src: k3,
...
export default function About () {
  ...
  return (
    <div className="about-main flex-col-center-start w-100 dark">
      <h1 className="mt-3">Hi! I'm Kaye</h1>
      <div className="page mt-4 flex-row-start-between w-70">
        <img className='w-50' src={obj[sel].src} alt="" />
        <span className='ml-2'>{obj[sel].msg + lorem }</span>
      </div>
      <div className="foot flex-row-center-center">
        <div onClick={() => setSel(0)} className={`circle m-0-1 ${sel === 0 && "active"}`}></div>
        <div onClick={() => setSel(1)} className={`circle m-0-1 ${sel === 1 && "active"}`}></div>
        <div onClick={() => setSel(2)} className={`circle m-0-1 ${sel === 2 && "active"}`}></div>
        <div onClick={() => setSel(3)} className={`circle m-0-1 ${sel === 3 && "active"}`}></div>
        <div onClick={() => setSel(4)} className={`circle m-0-1 ${sel === 4 && "active"}`}></div>
      </div>
    </div>
  )
}